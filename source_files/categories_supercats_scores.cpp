/**************** categories_supercats_scores.cpp
	...
	Note: This program requires C++11 language and library support. Compile with -std=c++11. On Macs with clang++ also use -stdlib=libc++.
			This program will run faster with OpenMP support. Compile with -fopenmp. 
...
void populate_graph_matrix(unordered_map<string, node*> &lookup_table, sp_fmat *category_matrix, float alpha) {
	...
	// First build a vector containing the list of category names. Although this uses a lot of memory, it enables parallelization via OpenMP. 
	vector<string> v;
	v.reserve(lookup_table.size());
	...
	// Parallelize this loop. Note: Armadillo uses OpenBLAS, which must be compiled with OpenMP-supporting flags. 
#pragma omp parallel for
	for (auto it = v.begin(); it < v.end(); it++) {

		node *n = lookup_table.at(*it);
