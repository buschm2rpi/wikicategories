...
// Used in the BFS
const string top_category = "Main_topic_classifications";
...
// Annotate each reachable node in the tree with the distance to each of the children of the specified top node. 
void tree_annotate(const string &topnodename, unordered_map<string, node *> &lookup_table) {
	...
	for (node *main_category : top->children) {

		string main_category_name = main_category->name;

		cout << "\tAnnotating for category " << main_category_name << endl;
...
int main(int argc, char ** argv) {
	...
	cout << "supercat_lookup_all_caching: lookup supercategories of a wikipedia category\n";
	cout << "\tReading from categories_parsed.txt generated by skos_parser.sh\n";
	...
	cout << "Performing tree annotation for all categories...\n";
	tree_annotate(top_category, lookup_table);

	// Output all categories and their annotated values
	tree_dump_annotations(top_category, lookup_table);
}
